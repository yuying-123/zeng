...
def main():
    ...
    if df is not None:
        ...
        if 'selected_company' not in st.session_state:
            # 设置默认选择
            first_company = df['企业名称'].unique()[0]
            first_stock = df[df['企业名称'] == first_company]['股票代码'].iloc[0]
            first_year = sorted(df['年份'].unique())[0]
            
            st.session_state.selected_company = first_company
            st.session_state.selected_stock = first_stock
        ...
        with st.form("查询表单"):
            ...
            if search_method == '股票代码':
                selected_stock = st.selectbox("选择股票代码", df['股票代码'].unique(), 
                                            index=list(df['股票代码'].unique()).index(st.session_state.selected_stock))
                # 获取对应的企业名称
                company_name = df[df['股票代码'] == selected_stock]['企业名称'].iloc[0]
            else:
                selected_company = st.selectbox("选择企业名称", df['企业名称'].unique(), 
                                              index=list(df['企业名称'].unique()).index(st.session_state.selected_company))
            ...
            selected_year = st.selectbox("选择年份", year_options, 
                                       index=list(year_options).index(st.session_state.selected_year))
        ...
        # 确保公司数据存在
        if not df[df['企业名称'] == company_name].empty:
            # 显示查询结果
